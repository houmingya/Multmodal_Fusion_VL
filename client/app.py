# ====================================
# 本地端代码 - Gradio可视化界面
# ====================================
# 功能：图文问答 + 文搜图 交互界面
# 环境：本地PC + Python 3.10+
# 依赖：gradio, requests, Pillow

import gradio as gr
import requests
import base64
from io import BytesIO
from PIL import Image
import json

# ====================================
# 配置项（根据服务器实际地址修改）
# ====================================
SERVER_URL = "http://localhost:8000"  # 服务器地址（修改为实际IP:端口）

# ====================================
# 工具函数
# ====================================
def check_server_health():
    """
    检查服务器连接状态
    返回：连接状态消息
    """
    try:
        response = requests.get(f"{SERVER_URL}/health", timeout=5)
        if response.status_code == 200:
            data = response.json()
            return f"✓ 服务器连接正常\n设备: {data['device']}\n图片库: {data['image_library_size']}张"
        else:
            return f"✗ 服务器响应异常 (状态码: {response.status_code})"
    except requests.exceptions.ConnectionError:
        return "✗ 无法连接到服务器，请检查：\n1. 服务器是否已启动\n2. SERVER_URL配置是否正确\n3. 网络/防火墙设置"
    except Exception as e:
        return f"✗ 连接失败: {str(e)}"

# ====================================
# 功能1：图文问答（VQA）
# ====================================
def vqa_inference(image, question):
    """
    图文问答功能
    输入：PIL图片对象 + 中文问题
    输出：模型回答文本
    """
    # 1. 输入验证
    if image is None:
        return "⚠ 请先上传图片！"
    
    if not question or question.strip() == "":
        return "⚠ 请输入问题！"
    
    try:
        # 2. 将PIL图片转为字节流
        img_byte_arr = BytesIO()
        image.save(img_byte_arr, format='JPEG')
        img_byte_arr.seek(0)
        
        # 3. 构造HTTP请求
        files = {
            'image': ('image.jpg', img_byte_arr, 'image/jpeg')
        }
        data = {
            'question': question.strip()
        }
        
        # 4. 发送POST请求到服务器
        response = requests.post(
            f"{SERVER_URL}/vqa",
            files=files,
            data=data,
            timeout=60  # VQA推理较慢，设置60秒超时
        )
        
        # 5. 解析返回结果
        if response.status_code == 200:
            result = response.json()
            answer = result.get('answer', '未返回答案')
            return f"🤖 模型回答：\n\n{answer}"
        else:
            error_detail = response.json().get('detail', '未知错误')
            return f"✗ 推理失败: {error_detail}"
            
    except requests.exceptions.Timeout:
        return "✗ 请求超时，服务器可能负载过高或模型推理较慢，请稍后重试"
    except requests.exceptions.ConnectionError:
        return "✗ 无法连接到服务器，请检查服务器是否运行"
    except Exception as e:
        return f"✗ 发生错误: {str(e)}"

# ====================================
# 功能2：文搜图（Text-to-Image Search）
# ====================================
def text2image_search(text_query, top_k):
    """
    文搜图功能
    输入：中文文本描述 + 返回数量
    输出：最相似的图片列表
    """
    # 1. 输入验证
    if not text_query or text_query.strip() == "":
        return [], "⚠ 请输入检索文本！"
    
    try:
        # 2. 构造HTTP请求
        data = {
            'text_query': text_query.strip(),
            'top_k': int(top_k)
        }
        
        # 3. 发送POST请求
        response = requests.post(
            f"{SERVER_URL}/text2image_search",
            data=data,
            timeout=30
        )
        
        # 4. 解析返回结果
        if response.status_code == 200:
            result = response.json()
            results = result.get('results', [])
            
            if not results:
                return [], "未找到匹配的图片"
            
            # 5. 解码Base64图片
            images = []
            info_text = f"🔍 检索结果（共{len(results)}张）：\n\n"
            
            for i, item in enumerate(results, 1):
                # 解码Base64图片
                # 检查字段是否存在
                if 'image_base64' not in item:
                    return [], f"✗ 服务器响应格式错误，缺少image_base64字段。实际字段: {list(item.keys())}"
                img_data = base64.b64decode(item['image_base64'])
                img = Image.open(BytesIO(img_data))
                images.append(img)
                
                # 构造信息文本
                info_text += f"{i}. {item['image_name']}\n"
                info_text += f"   相似度: {item['similarity_score']:.4f}\n\n"
            
            return images, info_text
        else:
            error_detail = response.json().get('detail', '未知错误')
            return [], f"✗ 检索失败: {error_detail}"
            
    except requests.exceptions.Timeout:
        return [], "✗ 请求超时，请稍后重试"
    except requests.exceptions.ConnectionError:
        return [], "✗ 无法连接到服务器，请检查服务器是否运行"
    except Exception as e:
        return [], f"✗ 发生错误: {str(e)}"

# ====================================
# Gradio界面构建
# ====================================
def build_interface():
    """
    构建Gradio可视化界面
    包含两个标签页：图文问答 + 文搜图
    """
    
    # 自定义CSS样式（支持中文显示）
    custom_css = """
    .gradio-container {
        font-family: "Microsoft YaHei", "SimHei", sans-serif !important;
    }
    """
    
    with gr.Blocks(
        title="多模态融合Demo - 图文问答+文搜图",
        css=custom_css,
        theme=gr.themes.Soft()
    ) as demo:
        
        # 标题和说明
        gr.Markdown(
            """
            # 🚀 多模态融合Demo - 基于魔搭社区
            
            **功能**：图文问答（VQA） + 文搜图（Text-to-Image Search）
            
            **模型**：LLaVA-1.5-7B + CLIP中文轻量版
            
            ---
            """
        )
        
        # 服务器状态检查
        with gr.Row():
            server_status = gr.Textbox(
                label="📡 服务器连接状态",
                value="点击右侧按钮检查连接...",
                interactive=False,
                lines=3
            )
            check_btn = gr.Button("🔄 检查连接", size="sm")
        
        check_btn.click(
            fn=check_server_health,
            inputs=[],
            outputs=server_status
        )
        
        # ====================================
        # 标签页1：图文问答
        # ====================================
        with gr.Tab("📷 图文问答（VQA）"):
            gr.Markdown("### 上传图片并提问，模型将基于图片内容回答您的问题")
            
            with gr.Row():
                with gr.Column(scale=1):
                    vqa_image = gr.Image(
                        label="上传图片",
                        type="pil",
                        height=300
                    )
                    vqa_question = gr.Textbox(
                        label="输入您的问题（中文）",
                        placeholder="例如：图片中有什么？这是什么场景？",
                        lines=3
                    )
                    vqa_submit_btn = gr.Button("🚀 提交问答", variant="primary")
                
                with gr.Column(scale=1):
                    vqa_answer = gr.Textbox(
                        label="模型回答",
                        lines=10,
                        interactive=False
                    )
            
            # 示例问题
            gr.Examples(
                examples=[
                    ["图片中有什么？"],
                    ["描述一下这张图片"],
                    ["图片中的主要物体是什么？"],
                    ["这是什么场景？"]
                ],
                inputs=vqa_question,
                label="💡 示例问题"
            )
            
            # 绑定事件
            vqa_submit_btn.click(
                fn=vqa_inference,
                inputs=[vqa_image, vqa_question],
                outputs=vqa_answer
            )
        
        # ====================================
        # 标签页2：文搜图
        # ====================================
        with gr.Tab("🔍 文搜图（Text-to-Image Search）"):
            gr.Markdown("### 输入文字描述，从服务器图片库中检索最匹配的图片")
            
            with gr.Row():
                with gr.Column(scale=1):
                    search_text = gr.Textbox(
                        label="输入检索文本（中文）",
                        placeholder="例如：一只可爱的猫、美丽的日落、狗在草地上",
                        lines=3
                    )
                    search_top_k = gr.Slider(
                        label="返回图片数量",
                        minimum=1,
                        maximum=10,
                        value=3,
                        step=1
                    )
                    search_btn = gr.Button("🔍 开始检索", variant="primary")
                    
                    search_info = gr.Textbox(
                        label="检索结果信息",
                        lines=8,
                        interactive=False
                    )
            
            with gr.Row():
                search_gallery = gr.Gallery(
                    label="匹配的图片（按相似度排序）",
                    columns=3,
                    height=400
                )
            
            # 示例文本
            gr.Examples(
                examples=[
                    ["一只可爱的猫"],
                    ["美丽的日落风景"],
                    ["狗在草地上奔跑"],
                    ["城市夜景"]
                ],
                inputs=search_text,
                label="💡 示例检索文本"
            )
            
            # 绑定事件
            search_btn.click(
                fn=text2image_search,
                inputs=[search_text, search_top_k],
                outputs=[search_gallery, search_info]
            )
        
        # 页脚说明
        gr.Markdown(
            """
            ---
            
            ### 📌 使用说明
            
            **图文问答**：
            1. 上传一张图片（支持jpg/png/webp等格式）
            2. 输入您想问的问题（中文）
            3. 点击"提交问答"按钮
            4. 等待服务器推理（约5-15秒）
            
            **文搜图**：
            1. 输入想检索的文字描述（中文）
            2. 选择返回图片数量
            3. 点击"开始检索"按钮
            4. 查看匹配结果和相似度分数
            
            ### ⚙️ 配置服务器地址
            
            请在代码第15行修改 `SERVER_URL` 变量：
            ```python
            SERVER_URL = "http://您的服务器IP:8000"
            ```
            
            ### ❓ 常见问题
            
            - **连接失败**：检查服务器是否启动，防火墙是否开放8000端口
            - **推理超时**：首次推理需要加载模型，耐心等待60秒
            - **图片库为空**：在服务器端 `image_library/` 目录添加图片
            """
        )
    
    return demo

# ====================================
# 启动Gradio界面
# ====================================
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("  多模态融合客户端启动中...")
    print("=" * 60)
    print(f"  服务器地址: {SERVER_URL}")
    print("  如需修改，请编辑代码第15行的 SERVER_URL 变量")
    print("=" * 60 + "\n")
    
    demo = build_interface()
    
    # 启动Gradio（自动打开浏览器）
    demo.launch(
        server_name="127.0.0.1",  # 本地访问
        server_port=7860,          # Gradio默认端口
        share=False,               # 不生成公网链接
        inbrowser=True             # 自动打开浏览器
    )
